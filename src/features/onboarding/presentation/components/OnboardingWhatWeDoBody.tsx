...
type OnboardingWhatWeDoBodyProps = {
  /**
   * Tapping the "Let's go!" CTA on the third slide. Parent routes into
   * the create-account screen via `router.replace` so the carousel is
...
/**
 * Pure presentation layer for the onboarding "what we do" carousel.
 * Renders three swipeable slides — Welcome, Stand Out, Trust — inside
 * `PaperOnboarding`, ending on a "Let's go!" CTA that advances the user
 * into the create-account screen.
 *
 * Reads no feature hooks. The connected screen
 * (`OnboardingWhatWeDoScreen`) wraps this with `useOnboardingWhatWeDo`
 * and passes the finish callback in. Stories render this body directly
 * with a stub — no router mocks needed.
 */
export const OnboardingWhatWeDoBody = ({
  onFinish,
}: OnboardingWhatWeDoBodyProps) => {
